// app/personal/day/[date]/DayContent.tsx
'use client';
...
export default function DayContent() {
  ...
  const { modeData, updateModeData, isLoaded } = useAppData('personal');
  ...
  const [newItem, setNewItem] = useState({
    ...
    quantity: 1,
    unit: 'পিস',
    price: 0,      // এখন এটি মোট মূল্য (total price)
    category: '',
  });
  ...
  useEffect(() => {
    ...
    if (foundMonth) {
      setMonth(foundMonth);
      const expenses = foundMonth.expenses.filter((exp) => exp.date.split('T')[0] === date);
  ...
  }, [monthId, date, personalData, router, isLoaded]);
  ...
  const addExpenseItem = () => {
    ...
    const item: ExpenseItem = {
      ...
      unit: newItem.unit,
      price: newItem.price, // এখন এটি মোট মূল্য
      category: newItem.category,
    };

    const existingExpense = dayExpenses.find(e => e.date.split('T')[0] === date && e.note === 'দৈনিক খরচ');

    let updatedExpenses = [...month.expenses];
    // মূল্য গণনা: এখন price-ই মোট, quantity দিয়ে গুণ করা হচ্ছে না
    const totalItemPrice = newItem.price;
    ...
    if (updatedMonth.categories) {
      updatedMonth.categories = updatedMonth.categories.map((cat) => {
        if (cat.name === newItem.category) {
          return { ...cat, spent: cat.spent + totalItemPrice };
        }
        return cat;
      });
    }
    ...
    setNewItem({ name: '', quantity: 1, unit: 'পিস', price: 0, category: '' });
  ...
  const saveSpecialEvent = () => {
    ...
    const updatedMonth = {
      ...month,
      specialEvents: { ...month.specialEvents, [date]: specialEvent },
    };
  ...
  const updatePersonalData = (updatedMonth: MonthData) => {
    ...
    if (personalData.currentMonth?.id === monthId) {
      updateModeData({ ...personalData, currentMonth: updatedMonth });
    } else {
      const updatedHistory = personalData.monthHistory?.map((m) =>
        m.id === monthId ? updatedMonth : m
      ) || [];
      updateModeData({ ...personalData, monthHistory: updatedHistory });
    }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{formattedDate}</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">{month.name}</p>
        </div>
      </div>
      ...
      <div className={cardClasses}>
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">আজকের খরচ</h3>
          <button
        ...
        {showAddForm && (
          <div className="mb-6 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
            <h4 className="font-medium text-gray-900 dark:text-white mb-3">নতুন খরচের আইটেম</h4>
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2">
                <input
                  type="text"
                  placeholder="আইটেমের নাম (যেমন: ডিম)"
                  value={newItem.name}
        ...
        {dayExpenses.length > 0 ? (
                  ...
                  >
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-500 dark:text-gray-400">
                        {item.quantity} {item.unit} - ৳{item.price} (মোট)
                      </div>
                    </div>
          ...
          <p className="text-center text-gray-500 dark:text-gray-400 py-8">আজকের জন্য কোনো খরচ নেই</p>
        )}
      </div>
